...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        _saperaters = [NSMutableArray array];
        [self commonInit];
    }
    return self;
}
...
//        }
        ...
        if (self.isAgeBand) {
            ...
            [self setSelectedBarColor:[UIColor darkGrayColor] forSegmentAtIndex:0];
        }
        
    }];
    
    self.selectionIndicator.frame = [self selectionIndicatorRect];
    [self.selectionIndicator setBackgroundColor:[[self barColors] objectAtIndex:self.selectedSegmentIndex]];
    _hairline.frame = [self hairlineRect];
    
    [self bringSubviewToFront:self.selectionIndicator];
    [self.selectionIndicator removeFromSuperview]; // not required right now ..
...
- (UIColor *)titleColorForState:(UIControlState)state
{
    ...
    if (!color) {
        switch (state) {
            ...
            case UIControlStateSelected:            return [UIColor whiteColor];
...
- (void)setCount:(NSNumber *)count forSegmentAtIndex:(NSUInteger)segment
{
    ...
    [self configureSegments];
    if (self.barColors.count < segment+1) {
        [[self barColors] insertObject:self.tintColor atIndex:segment];
    }
}
...
- (void)setSelectedBarColor:(UIColor *)barColor forSegmentAtIndex:(NSInteger)segment {
    NSAssert(segment < self.numberOfSegments, @"Cannot assign a tint color to non-existing segment.");
    NSAssert(segment >= 0, @"Cannot assign a tint color to a negative segment.");
    
    NSAssert([barColor isKindOfClass:[UIColor class]], @"Cannot assign a tint color with an unvalid color object.");
    [[self barColors] replaceObjectAtIndex:segment withObject:barColor];
}
...
- (NSMutableArray *)barColors {
    if (!_barColors) {
        _barColors = [NSMutableArray arrayWithCapacity:self.items.count];
        
        for (NSInteger idx = 0; idx < self.items.count; idx++) {
            [_barColors addObject:self.tintColor];
        }
    }
    return _barColors;
}
...
- (void)addButtonForSegment:(NSUInteger)segment
{
    ...
    if (segment < self.items.count) {
        ...
        [self.saperaters addObject:line];
