...
@interface WSAssetTableViewController () <WSAssetsTableViewCellDelegate,QLPreviewControllerDataSource,QLPreviewControllerDelegate>
{
    UIBarButtonItem *selectBarButton;
    UIBarButtonItem *previewBarButton;
...
- (void)viewDidLoad
{
    ...
    selectBarButton=[[UIBarButtonItem alloc]initWithTitle:@"Select" style:UIBarButtonItemStyleBordered target:self action:@selector(select)];
    
    previewBarButton=[[UIBarButtonItem alloc]initWithTitle:@"Preview" style:UIBarButtonItemStyleBordered target:self action:@selector(select)];
    

    donebarButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone
                                                                                           target:self 
                                                                                           action:@selector(doneButtonAction:)];
    self.navigationItem.rightBarButtonItems=@[donebarButton,previewBarButton];
...
-(void)select
{
    if(self.isSelected)
    {
        ...
        self.navigationItem.rightBarButtonItems=@[donebarButton,selectBarButton];
        self.isSelected=NO;
...
- (void)assetsTableViewCell:(WSAssetsTableViewCell *)cell didSelectAsset:(BOOL)selected atColumn:(NSUInteger)column
{
    ...
    if(!self.isSelected)
    {
       ...
       // self.currentFileURL=assetWrapper.asset.defaultRepresentation.url;
        ALAssetRepresentation *defaultRepresentation = [assetWrapper.asset defaultRepresentation];
        NSString *uti = [defaultRepresentation UTI];
        NSURL  *videoURL = [[assetWrapper.asset valueForProperty:ALAssetPropertyURLs] valueForKey:uti];
        self.currentFileURL=videoURL;
        ...
        if ([[assetWrapper.asset valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo])
        {
            ...
            [moviePlayer.navigationController.navigationBar setTranslucent:NO];
        ...
        else if ([[assetWrapper.asset valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypePhoto])
        {
            ALAssetRepresentation *rep = [assetWrapper.asset defaultRepresentation];
            CGImageRef iref = [rep fullResolutionImage];
            if (iref)
            {
                UIImage *largeimage = [UIImage imageWithCGImage:iref];
                ShowImageViewController *viewController=[[ShowImageViewController alloc]initWithNibName:NSStringFromClass([ShowImageViewController class]) bundle:nil];
...
{
    if([self.navigationController.viewControllers containsObject:moviePlayer])
    {
