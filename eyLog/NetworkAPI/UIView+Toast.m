...
- (void)makeToast:(NSString *)message {
    ...
    [self makeToast:message duration:CSToastDefaultDuration position:CSToastPositionBottom];
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position {

    AppDelegate *app=(AppDelegate *)[UIApplication sharedApplication].delegate;
    NSLog(@"Seconds --------> %f",[[NSDate date] timeIntervalSinceDate: app.interval]);
    float sec=[[NSDate date] timeIntervalSinceDate:app.interval];
    if (sec<0.7) {
        NSLog(@"Return ");
        app.interval=[NSDate date];
        return;
    }
    app.interval=[NSDate date];
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    if([self.subviews containsObject:toast]) {
...
-(void)hideToastFromView{

     UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &CSToastTimerKey);
    NSLog(@"subviews %@",existingActivityView);
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        if([point caseInsensitiveCompare:CSToastPositionTop] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width/2, (toast.frame.size.height / 2) + CSToastVerticalPadding+120);}
